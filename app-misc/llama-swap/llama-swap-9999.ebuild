...
src_compile() {
	local GIT_HASH
	GIT_HASH="$(git rev-parse --short HEAD)" || die "failed to get git hash"
	...
	CGO_ENABLED=0 go build \
		-ldflags="-X main.version=local_${GIT_HASH} -X main.commit=${GIT_HASH} -X main.date=$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
...
pkg_postinst() {
	elog "llama-swap (live git) has been installed to /usr/bin/llama-swap"
	elog ""
